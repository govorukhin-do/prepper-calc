...
export default function App() {
  ...
  const groupedAutoContainers = useMemo(() => {
    const groups: { [key: string]: { container: PackedContainer, count: number, ids: string[], isFull: boolean } } = {};
    activeContainers.forEach(c => {
      ...
      if (!groups[key]) {
        ...
        const usedUnits = c.packetIds.reduce((acc, pId) => acc + SIZE_UNITS[FOOD_PACKETS.find(pkt => pkt.id === pId)!.size], 0);
        const maxUnits = type.mCapacity * 6 + type.extraSCapacity;
        const isFull = usedUnits >= maxUnits;
        groups[key] = { container: c, count: 0, ids: [], isFull };
      }
      groups[key].count++;
  ...
  return (
    <div className="min-h-screen bg-tactical-bg text-tactical-text tactical-grid pb-20">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        {/* Tabs */}
        <div ref={tabsRef} className="flex gap-2 mb-8 bg-tactical-card p-1 rounded-lg border border-tactical-border scroll-mt-20">
          {[
                    ...
                    <div className="lg:col-span-2 space-y-4">
                      ...
                      {activeContainers.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {groupedAutoContainers.map((group) => {
                            ...
                            return (
                              <div key={group.container.id} className="bg-tactical-card border border-tactical-border rounded-xl flex flex-col relative">
                                {group.count > 1 && (
